...
static void HAL_PCD_MspInit(PCD_HandleTypeDef* hpcd)
#else
...
#if (USE_HAL_PCD_REGISTER_CALLBACK == 1U)
static void HAL_PCD_MspDeInit(PCD_HandleTypeDef* hpcd)
#else
...
#endif /* USE_HAL_PCD_REGISTER_CALLBACKS */
{
      ...
      (((USBD_HandleTypeDef*)hpcd->pData)->dev_old_state == USBD_STATE_CONFIGURED))
  {
    ...
    /* Select Stop 0 mode */
    MODIFY_REG(PWR->CR1, PWR_CR1_LPMS_Pos, PWR_CR1_LPMS_Msk);
    /* Set SLEEPDEEP bit of Cortex System Control Register. */
    SCB->SCR |= (uint32_t)(SCB_SCR_SLEEPONEXIT_Msk);
    HAL_PWR_EnableSleepOnExit();
#elif defined USBD_LPM_STOP1_CONFIG
    /* Stop 1 mode */
    MODIFY_REG(PWR->CR1, PWR_CR1_LPMS, PWR_CR1_LPMS_0);
    /* Set SLEEPDEEP bit of Cortex System Control Register. */
    SCB->SCR |= (uint32_t)(SCB_SCR_SLEEPONEXIT_Msk);
    HAL_PWR_EnableSleepOnExit();
#elif defined USBD_LPM_STOP2_CONFIG
    /* Stop 2 mode */
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  * application, it is advised to not enable this option unless required. */
  ...
  hpcd_USB_DRD_FS.Init.dev_endpoints = 9;
