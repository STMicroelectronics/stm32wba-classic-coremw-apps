...
Communication Class (CDC) following the PSTN sub-protocol on the NUCLEO-WBA25CE devices.

This example is part of the USB Device Library package using STM32Cube firmware.
This is a typical application on how to use the STM32WBA25xx USB Device peripheral where the STM32 MCU
...
When plugged to PC host, the NUCLEO-WBA25CE must be properly enumerated as a USB Serial device and an STlink COM port.
...
To run this application, the user can use one of the following configurations:
 ...
 - Configuration 2:
   Connect USB cable to Host and connect UART TX pin to UART RX pin on the NUCLEO-WBA25CE board
...
### <b>Hardware and Software environment</b>
 ...
 - NUCLEO-WBA25CE board Set-up
   - Connect the NUCLEO-WBA25CE board CN15 to the PC through "TYPE-C" to "Standard A" cable.
   - Connect the NUCLEO-WBA25CE board to the PC (or to another evaluation board) through USART
     serial cable CN9 connector.
   - For loopback mode test: connect directly USART TX and RX pins:
     PA6 and PA12

 The USART1 interface available on PA6 and PA12 of the microcontroller can be
