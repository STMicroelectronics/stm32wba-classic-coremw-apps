...
/**
  ******************************************************************************
  * @file    USB_Host/CDC_Standalone/USB_Host/cdc_send.c
...
{
  if (hUsbHostHS.pActiveClass == USBH_CDC_CLASS)
  {
    /* Start Sending data */
    USBH_UsrLog("Sending data ...");
    USBH_StatusTypeDef status = USBH_CDC_Transmit(&hUsbHostHS, CDC_TX_Buffer, sizeof(CDC_TX_Buffer));
    if (status == USBH_OK)
    {
