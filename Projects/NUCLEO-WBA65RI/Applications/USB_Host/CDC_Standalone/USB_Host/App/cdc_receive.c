...
/**
  ******************************************************************************
  * @file    USB_Host/CDC_Standalone/USB_Host/cdc_receive.c
...
{
  USBH_CDC_Receive(&hUsbHostHS, CDC_RX_Buffer, RX_BUFF_SIZE);
  DumpReceivedData();
}
...
static void DumpReceivedData(void)
{
  ...
  uint16_t count = 0;
  size = USBH_CDC_GetLastReceivedDataSize(&hUsbHostHS);
