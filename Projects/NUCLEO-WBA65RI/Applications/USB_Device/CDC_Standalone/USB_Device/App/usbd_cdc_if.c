...
static int8_t CDC_Init_HS(void)
{
  ...
  MX_GPDMA1_Init();
  MX_USART1_UART_Init();
  if(HAL_UART_Receive_IT(&huart1, (uint8_t *)UserTxBufferHS, 1) != HAL_OK)
  {
...
static void ComPort_Config(void)
{
  ...
  /* Start reception: provide the buffer pointer with ofHSet and the buffer size */
  HAL_UART_Receive_IT(&huart1, (uint8_t *)(UserTxBufferHS + UserTxBufPtrIn), 1);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  /* Start another reception: provide the buffer pointer with ofHSet and the buffer size */
  HAL_UART_Receive_IT(huart, (uint8_t *)(UserTxBufferHS + UserTxBufPtrIn), 1);
  }
