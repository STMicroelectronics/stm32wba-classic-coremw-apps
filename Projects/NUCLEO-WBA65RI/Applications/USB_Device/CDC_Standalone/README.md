...
Communication Class (CDC) following the PSTN sub-protocol on the NUCLEO-WBA65RI devices.

This example is part of the USB Device Library package using STM32Cube firmware.
This is a typical application on how to use the STM32WBA65xx USB Device peripheral where the STM32 MCU
...
implementation (PSTN sub-class):
 - 1 x Bulk IN endpoint for receiving data from STM32 device to PC host:
   ...
   timer callback the state of the buffer "UserTxBufferHS" is checked. If there are available data, they
   are transmitted in response to IN token otherwise it is NAKed.

 - 1 x Bulk OUT endpoint for transmitting data from PC host to STM32 device:
   When data are received through this endpoint they are saved in the buffer "UserRxBufferHS" then they
 ...
 - 1 x Interrupt IN endpoint for setting and getting serial-port parameters:
   When control setup is received, the corresponding request is executed in CDC_Control_HS().
...
When plugged to PC host, the NUCLEO-WBA65RI must be properly enumerated as a USB Serial device and an STlink COM port.
...
To run this application, the user can use one of the following configurations:
 ...
 - Configuration 2:
   Connect USB cable to Host and connect UART TX pin to UART RX pin on the NUCLEO-WBA65RI board
...
### <b>Hardware and Software environment</b>
 ...
 - NUCLEO-WBA65RI board Set-up
   - Connect the NUCLEO-WBA65RI board CN15 to the PC through "TYPE-C" to "Standard A" cable.
   - Connect the NUCLEO-WBA65RI board to the PC (or to another evaluation board) through USART
     serial cable CN9 connector.
   - For loopback mode test: connect directly USART TX and RX pins:
     PA8 and PB12

 The USART1 interface available on PA8 and PB12 of the microcontroller can be
